---
description: Thorough review of a PR given a GitHub PR URL.
agent: pr-review-full
---
...
If my request includes an explicit instruction to approve the PR (e.g. "approve this PR", "please approve", "LGTM approve it"), pass that approval instruction through to the `pr-review-full` agent verbatim along with the URL — do not strip it. Also pass the configured review label above to the agent so it can remove it on approval. A plain review request with no approval language is just a review.
